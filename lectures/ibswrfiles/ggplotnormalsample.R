...
pp <- ggplot() + theme_bw() + theme(text = element_text(size = 18)) +
    geom_histogram(data = df, mapping = aes(x = x, y = after_stat(density)), bins = 30, colour = "darkorange", fill = "darkorange", alpha = 0.33) +
...
if (internalFlag == TRUE) {
  pp <- pp + geom_vline(xintercept = local_mean, colour = "darkorchid4", linetype = 2, linewidth = 2)
  if (internalFlag2 == TRUE) {
    pp <- pp + geom_area(data = df2, mapping = aes(x = xs, y = ys), colour = "darkorchid4", linewidth = 1, fill = "darkorchid4", alpha = 0.33)
  } else {
    pp <- pp +
      ...
      geom_segment(aes(x = local_mean, xend = local_mean - local_sd, y = exp(-1/2), yend = exp(-1/2)), 
                 arrow = arrow(length = unit(0.5, "cm")), colour="firebrick3", linewidth = 2) +
      geom_segment(aes(x = local_mean, xend = local_mean, y = 0.95*exp(-1/2), yend = 1.05*exp(-1/2)), colour = "firebrick3", linewidth = 2)
  }
} else {
  if (internalFlag2 == TRUE) {
